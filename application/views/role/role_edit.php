...
<div class="layui-fluid">
    <div class="layui-row">
        <form method="post" class="layui-form layui-form-pane" id="tab">
					...
					<tr>
						<td>
							<input type="checkbox" <?php echo $role_status1 == 1 ?'checked':'' ?> name="menu[]" value="1" lay-skin="primary" lay-filter="father"
								   lay-verify="check" title="系统功能模块">
						</td>
						<td>
							<input name="menu[]" <?php echo $role_status2 == 1 ?'checked':'' ?> lay-skin="primary" type="checkbox" value="2" title="基础管理模块"
								   lay-verify="check" lay-filter="father">
						</td>
						<td>
							<input name="menu[]" <?php echo $role_status3 == 1 ?'checked':'' ?> lay-skin="primary" type="checkbox" value="3" title="文件管理模块"
								   lay-verify="check" lay-filter="switchTest">
						</td>
					</tr>
					</tbody>
				</table>
			...
			<div class="layui-form-item layui-form-text" id="op1" style="display: <?php echo $role_status3 == 1 ?'block':'none' ?>">
				<label class="layui-form-label">
					文件类型权限
				</label>
				<table class="layui-table layui-input-block">
					...
					<tr>
						<?php if (isset($typelist) && !empty($typelist)) { ?>
						    <?php foreach ($typelist as $k => $v) : ?>
						         <?php if ($k<5) { ?>
									<td>
										<input type="checkbox" <?php echo !empty($v['open']) == 1 ?'checked':'' ?> name="menu1[]" value="<?= $v['ftid'] ?>" lay-skin="primary" lay-filter="father"
											   lay-verify="check" title="<?= $v['fname'] ?>">
									</td>
								<?php } ?>
							<?php endforeach; ?>
						<?php } ?>
